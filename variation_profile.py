...
HOOK_TYPES = ("text", "before_after_image", "text_before_after_image", "b_roll", "text_b_roll")
...
def _render_preview_image(source: Path, output: Path, variant: dict[str, Any], index: int) -> None:
    ...
    label = _escape_drawtext(f"V{index + 1} {_hook_label(str(variant.get('hook_type') or 'text')).upper()}")

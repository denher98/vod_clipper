...
class VideoQueueRunner:
    ...
    def _variant_settings_overrides(self) -> dict:
        if self.pipeline_mode == "raw_cuts_only":
            return {"VARIANTS_PER_CLIP": 1}
        if self.variant_mode == "original":
            return {"VARIANTS_PER_CLIP": 1}
        if self.variant_mode == "custom":
            return {"VARIANTS_PER_CLIP": max(1, min(6, int(self.variant_count or 1)))}
        return {}
    ...
    def _enqueue_stage_locked(self, video_path: str, stage: str, reason: str) -> None:
        ...
        try:
            ...
            log.warning(
                "Stage queue %s is full at MAX_QUEUE_SIZE=%s; leaving %s stage=%s pending",
                queue_name,
                self.max_queue_size,
                Path(video_path).name,
                stage,

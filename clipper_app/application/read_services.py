...
class ReadDashboardService:
    def __init__(self, settings_provider: LegacyConfigProvider | None = None) -> None:
        ...
        self.cfg = self.settings_provider.config_module
    ...
    def module_library(
        ...
        *,
        ...
        rows = self._filter_module_rows(rows, search=search, status=status, product=product)
        ...
        for video in [self._aggregate_video_entry(item) for item in self._state_videos(state)]:
            for run in video.get("runs", []):
                if not isinstance(run, dict) or not run.get("output_dir"):
                    continue
                normalized = str(run["output_dir"])
                key = normalized.casefold()
                if key not in seen:
                    output_dirs.append(normalized)
                    seen.add(key)
                if len(output_dirs) >= max_dirs:
                    return tuple(output_dirs)
        root = self._output_root()
        if root.exists():
            try:
                folders = [folder for folder in root.iterdir() if folder.is_dir()]
                folders.sort(key=lambda folder: self._safe_mtime_ns(folder), reverse=True)
